 ...
 */
class auth_plugin_db_salt extends auth_plugin_base {
    ...
    function __construct() {
        ...
        $this->authtype = 'db_salt';
        $this->config = get_config('auth/db_salt');
    ...
    function user_login($username, $password) {
        ...
        //$extusername = mb_convert_encoding($username, 'utf-8', $this->config->extencoding);
        //$extpassword = mb_convert_encoding($password, 'utf-8', $this->config->extencoding);
        ...
        if ($this->is_internal()) {
            ...
            if ($this->config->passtype === 'md5') {   // Re-format password accordingly.
            ...
            } else if ($this->config->passtype === 'sha1salt') {
                $rssalt = $authdb->Execute("SELECT {$this->config->fieldsalt} FROM {$this->config->table} WHERE {$this->config->fielduser} = '".$this->ext_addslashes($extusername)."' ");          
                if (!$rssalt) {
                    $authdb->Close();
                    debugging(get_string('auth_dbcantconnect','auth_db'));
                    return false;
                ...
                if (!$rssalt->EOF) {         
                    $saltfield = $rssalt->fields['PasswordSalt'];
                    $rssalt->Close();
                    $salt = base64_decode($saltfield);
                    $utfpassword = mb_convert_encoding($extpassword,"UTF-16LE");
                    $extpassword = base64_encode(pack('H*', sha1($salt.$utfpassword)));
                } else {
                    $rssalt->Close();
                    $authdb->Close();
                    return false;
    ...
    function process_config($config) {
        ...
        // Save settings.
        set_config('host',          $config->host,          'auth/db_salt');
        set_config('type',          $config->type,          'auth/db_salt');
        set_config('sybasequoting', $config->sybasequoting, 'auth/db_salt');
        set_config('name',          $config->name,          'auth/db_salt');
        set_config('user',          $config->user,          'auth/db_salt');
        set_config('pass',          $config->pass,          'auth/db_salt');
        set_config('table',         $config->table,         'auth/db_salt');
        set_config('fielduser',     $config->fielduser,     'auth/db_salt');
        set_config('fieldpass',     $config->fieldpass,     'auth/db_salt');
        set_config('passtype',      $config->passtype,      'auth/db_salt');
        set_config('fieldsalt',     $config->fieldsalt,     'auth/db_salt');
        set_config('extencoding',   trim($config->extencoding), 'auth/db_salt');
        set_config('setupsql',      trim($config->setupsql),'auth/db_salt');
        set_config('debugauthdb',   $config->debugauthdb,   'auth/db_salt');
        set_config('removeuser',    $config->removeuser,    'auth/db_salt');
        set_config('changepasswordurl', trim($config->changepasswordurl), 'auth/db_salt');
