...
class auth_db_testcase extends advanced_testcase {
    ...
    protected function init_auth_database() {
        global $DB, $CFG;
        require_once("$CFG->dirroot/auth/db_salt/auth.php");

        $dbman = $DB->get_manager();

        set_config('extencoding', 'utf-8', 'auth/db_salt');

        set_config('host', $CFG->dbhost, 'auth/db_salt');
        set_config('user', $CFG->dbuser, 'auth/db_salt');
        set_config('pass', $CFG->dbpass, 'auth/db_salt');
        set_config('name', $CFG->dbname, 'auth/db_salt');

        if (!empty($CFG->dboptions['dbport'])) {
            set_config('host', $CFG->dbhost.':'.$CFG->dboptions['dbport'], 'auth/db_salt');
        }

        switch (get_class($DB)) {
            case 'mssql_native_moodle_database':
                set_config('type', 'mssql_n', 'auth/db_salt');
                set_config('sybasequoting', '1', 'auth/db_salt');
                break;

            case 'mariadb_native_moodle_database':
            case 'mysqli_native_moodle_database':
                set_config('type', 'mysqli', 'auth/db_salt');
                set_config('setupsql', "SET NAMES 'UTF-8'", 'auth/db_salt');
                set_config('sybasequoting', '0', 'auth/db_salt');
                if (!empty($CFG->dboptions['dbsocket'])) {
                    $dbsocket = $CFG->dboptions['dbsocket'];
                    if ((strpos($dbsocket, '/') === false and strpos($dbsocket, '\\') === false)) {
                        $dbsocket = ini_get('mysqli.default_socket');
                    }
                    set_config('type', 'mysqli://'.rawurlencode($CFG->dbuser).':'.rawurlencode($CFG->dbpass).'@'.rawurlencode($CFG->dbhost).'/'.rawurlencode($CFG->dbname).'?socket='.rawurlencode($dbsocket), 'auth/db_salt');
                }
                break;

            case 'oci_native_moodle_database':
                set_config('type', 'oci8po', 'auth/db_salt');
                set_config('sybasequoting', '1', 'auth/db_salt');
                break;

            case 'pgsql_native_moodle_database':
                set_config('type', 'postgres7', 'auth/db_salt');
                $setupsql = "SET NAMES 'UTF-8'";
                if (!empty($CFG->dboptions['dbschema'])) {
                    $setupsql .= "; SET search_path = '".$CFG->dboptions['dbschema']."'";
                }
                set_config('setupsql', $setupsql, 'auth/db_salt');
                set_config('sybasequoting', '0', 'auth/db_salt');
                if (!empty($CFG->dboptions['dbsocket']) and ($CFG->dbhost === 'localhost' or $CFG->dbhost === '127.0.0.1')) {
                    if (strpos($CFG->dboptions['dbsocket'], '/') !== false) {
                        set_config('host', $CFG->dboptions['dbsocket'], 'auth/db_salt');
                    } else {
                        set_config('host', '', 'auth/db_salt');
                    }
                }
                break;

            case 'sqlsrv_native_moodle_database':
                set_config('type', 'mssqlnative', 'auth/db_salt');
                set_config('sybasequoting', '1', 'auth/db_salt');
                break;
        ...
        $table = new xmldb_table('auth_db_salt_users');
        ...
        set_config('table', $CFG->prefix.'auth_db_salt_users', 'auth/db_salt');
        set_config('fielduser', 'name', 'auth/db_salt');
        set_config('fieldpass', 'pass', 'auth/db_salt');

        // Setu up field mappings.

        set_config('field_map_email', 'email', 'auth/db_salt');
        set_config('field_updatelocal_email', 'oncreate', 'auth/db_salt');
        set_config('field_updateremote_email', '0', 'auth/db_salt');
        set_config('field_lock_email', 'unlocked', 'auth/db_salt');

        // Init the rest of settings.
        set_config('passtype', 'plaintext', 'auth/db_salt');
        set_config('changepasswordurl', '', 'auth/db_salt');
        set_config('debugauthdb', 0, 'auth/db_salt');
        set_config('removeuser', AUTH_REMOVEUSER_KEEP, 'auth/db_salt');
    }
    ...
    public function test_plugin() {
        ...
        $user1->id = $DB->insert_record('auth_db_salt_users', $user1);
        ...
        $user2->id = $DB->insert_record('auth_db_salt_users', $user2);

        $user3 = (object)array('name'=>'admin', 'pass'=>'heslo', 'email'=>'admin@example.com'); // Should be skipped.
        $user3->id = $DB->insert_record('auth_db_salt_users', $user3);
        ...
        $DB->update_record('auth_db_salt_users', $user2b);
        ...
        set_config('field_updatelocal_email', 'onlogin', 'auth/db_salt');
        ...
        $DB->delete_records('auth_db_salt_users', array('id'=>$user2->id));
        $this->assertCount(2, $DB->get_records('auth_db_salt_users'));
        unset($user2);
        unset($user2b);
        ...
        set_config('removeuser', AUTH_REMOVEUSER_SUSPEND, 'auth/db_salt');
        ...
        set_config('removeuser', AUTH_REMOVEUSER_FULLDELETE, 'auth/db_salt');
        ...
        set_config('passtype', 'md5', 'auth/db_salt');
        $auth->config->passtype = 'md5';
        $user3->pass = md5('heslo');
        $DB->update_record('auth_db_salt_users', $user3);
        $this->assertTrue($auth->user_login('u3', 'heslo'));

        set_config('passtype', 'sh1', 'auth/db_salt');
        $auth->config->passtype = 'sha1';
        $user3->pass = sha1('heslo');
        $DB->update_record('auth_db_salt_users', $user3);
        $this->assertTrue($auth->user_login('u3', 'heslo'));

        set_config('passtype', 'internal', 'auth/db_salt');
        $auth->config->passtype = 'internal';
        create_user_record('u3', 'heslo', 'db');
        $this->assertTrue($auth->user_login('u3', 'heslo'));


        $DB->delete_records('auth_db_salt_users', array('id'=>$user3->id));
        ...
        $user4->id = $DB->insert_record('auth_db_salt_users', $user4);
        ...
        $DB->update_record('auth_db_salt_users', $user4b);
